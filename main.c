...
EXTI0_1_IRQHandler(void)
{
        ...
        exti_finite_state_machine(LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0), USER_BUTTON);
...
EXTI4_15_IRQHandler(void)
{
    ...
    if (LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_13)) {
        ...
        /* call exti_finite_state_machine */
        exti_finite_state_machine(LL_GPIO_IsInputPinSet(GPIOC, LL_GPIO_PIN_13), LEFT_BUTTON);
    ...
    /* Check, which button is pressed*/
    else if (LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_14)) {

        /* call exti_finite_state_machine */
        exti_finite_state_machine(LL_GPIO_IsInputPinSet(GPIOC, LL_GPIO_PIN_14), RIGHT_BUTTON);
...
exti_finite_state_machine(enum EDGE edge, enum BUTTONS button)
{
    switch (button)
    {
      case USER_BUTTON :
        {
            static enum BUTTON_STATES user_button_state =  BUTTON_STATES_DEFAULT;
            ...
            switch(left_button_state)
              {
                  case LEFT_BUTTON_RELEASED :
                      if ((edge == RISING) && (abs(TIME - left_button_timestamp) > 15))
                      {
                  ...
                  case LEFT_BUTTON_PRESSED :
                      if((edge == FALLING) && (abs(TIME - left_button_timestamp) > 15))
                      {
            ...
            switch(right_button_state)
              {
                  case RIGHT_BUTTON_RELEASED :
                      if ((edge == RISING) && (abs(TIME - right_button_timestamp) > 15))
                      {
                  ...
                  case RIGHT_BUTTON_PRESSED :
                      if((edge == FALLING) && (abs(TIME - right_button_timestamp) > 15))
                      {
                          right_button_state = LEFT_BUTTON_RELEASED;
                          right_button_timestamp = TIME;
                      }
                      break;
